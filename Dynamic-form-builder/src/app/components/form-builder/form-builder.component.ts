...
export class FormBuilderComponent {
  ...
  onDrop(event: CdkDragDrop<FormField[]>) {
    ...
    // Reset formFields and formGroup
    this.formFields = [];
    this.formGroup = this.fb.group({});
  
    // Reset availableFields to its original state
    this.availableFields = [
      { type: 'text', label: 'Text Input', name: 'textInput', id: '', required: false, helpText: 'Enter text here' },
      { type: 'textarea', label: 'Text Area', name: 'textArea', id: '', required: false, helpText: 'Enter long text here' },
      { type: 'dropdown', label: 'Dropdown', name: 'dropdown', options: ['Option 1', 'Option 2'], id: '', required: false },
      { type: 'checkbox', label: 'Checkbox', name: 'checkbox', id: '', required: false },
      { type: 'radio', label: 'Radio Buttons', name: 'radio', options: ['Option A', 'Option B'], id: '', required: false },
      { type: 'date', label: 'Date Picker', name: 'datePicker', id: '', required: false }
    ];
  }
  ...
  removeField(index: number) {
    ...
    // Add the field back to availableFields
    const originalField = this.availableFields.find(f => f.name.split('_')[0] === field.name.split('_')[0]);
    if (!originalField) {
      this.availableFields.push({
        ...field,
        name: field.name.split('_')[0], // Reset the name to its original value
        id: '',
        required: false
      });
    }
